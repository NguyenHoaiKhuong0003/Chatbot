...
import { faPaperPlane, faSpinner, faPlus, faMessage, faEllipsisVertical, faArchive } from "@fortawesome/free-solid-svg-icons";
...
const formatSourceName = (source) => {
  ...
  if (source.startsWith('http://') || source.startsWith('https://')) {
    ...
    // Treat as a local path (like data/10file1.pdf or data\10file1.pdf)
...
function ChatBot() {
  ...
  const suggestedPrompts = [
    ...
    },
    {
      ...
      description: "Tìm hiểu điều kiện tiên quyết của môn học"
    },
    {
      title: "Lịch học & Thời khóa biểu",
      description: "Xem thông tin về lịch học và thời khóa biểu"
    }
  ];
  ...
  const handleSendMessage = async (message) => {
    ...
    if (!user) {
      if (guestMessageCount >= GUEST_MESSAGE_LIMIT) {
        setState(prev => ({
          ...prev,
          error: "Bạn đã đạt giới hạn tin nhắn. Vui lòng đăng nhập để tiếp tục trò chuyện."
        }));
        return;
      }
    ...
    let timeoutId;
    try {
      // Add 10 second delay before sending the message
  ...
  const handleArchiveChat = async (e, chatId) => {
    ...
    try {
      ...
      setShowChatMenu(null);
      alert('Đã lưu trữ đoạn chat thành công!');
    } catch (error) {
      console.error('Error archiving chat:', error);
      alert('Có lỗi xảy ra khi lưu trữ đoạn chat. Vui lòng thử lại!');
    }
  };
            ...
            <div className="chat-history">
              {user ? (
                state.chatHistory && state.chatHistory.length > 0 ? (
                  state.chatHistory.map((conversation) => (
                    <div 
                      key={conversation.id}
